...
from typing import Tuple, Dict, Optional, TextIO
import os
...
class HistoricalWeather:
    ...
    def record_high(self, m: int, d: int) -> float:
        ...
        for a in self._records:
            if a.month == m and a.day == d:
                x.append(self._records[a].high_temp)
        return max(x)
    ...
    def monthly_average(self) -> Dict[str, float]:
        ...
        for a in self._records:
            if a.month == 1:
                jan.append(self._records[a].low_temp)
            elif a.month == 2:
                feb.append(self._records[a].low_temp)
            elif a.month == 3:
                mar.append(self._records[a].low_temp)
            elif a.month == 4:
                apr.append(self._records[a].low_temp)
            elif a.month == 5:
                may.append(self._records[a].low_temp)
            elif a.month == 6:
                jun.append(self._records[a].low_temp)
            elif a.month == 7:
                jul.append(self._records[a].low_temp)
            elif a.month == 8:
                aug.append(self._records[a].low_temp)
            elif a.month == 9:
                sep.append(self._records[a].low_temp)
            elif a.month == 10:
                october.append(self._records[a].low_temp)
            elif a.month == 11:
                nov.append(self._records[a].low_temp)
            elif a.month == 12:
                dec.append(self._records[a].low_temp)
        if len(jan) != 0:
            d['Jan'] = (sum(jan) / len(jan))
        if len(feb) != 0:
            d['Feb'] = (sum(feb) / len(feb))
        if len(mar) != 0:
            d['Mar'] = (sum(mar) / len(mar))
        if len(apr) != 0:
            d['Apr'] = (sum(apr) / len(apr))
        if len(may) != 0:
            d['May'] = (sum(may) / len(may))
        if len(jun) != 0:
            d['Jun'] = (sum(jun) / len(jun))
        if len(jul) != 0:
            d['Jul'] = (sum(jul) / len(jul))
        if len(aug) != 0:
            d['Aug'] = (sum(aug) / len(aug))
        if len(sep) != 0:
            d['Sep'] = (sum(sep) / len(sep))
        if len(october) != 0:
            d['Oct'] = (sum(october) / len(october))
        if len(nov) != 0:
            d['Nov'] = (sum(nov) / len(nov))
        if len(dec) != 0:
            d['Dec'] = (sum(dec) / len(dec))
        return d
    ...
    def contiguous_precipitation(self) -> tuple[date, int]:
        ...
        y = []
        for a in self._records:
            if self._records[a].precipitation > 0 or \
                    self._records[a].precipitation == -1:
                w.append(a)
        for i in range(len(w)):
            if i + 1 != len(w) or i != 0 and w[i].day - 1 == w[i - 1].day:
                x.append(w[i])
                x.sort()
        y.append(x[0])
        y.append(len(x))
    ...
    def percentage_snowfall(self) -> float:
        ...
        y = []
        for a in self._records:
            x.append(self._records[a].snowfall)
            y.append(self._records[a].rainfall)
...
Rain: 0
        """
        data = ''
        for a in self._histories:
            b = str(self._histories[a])
            data += b
        return f'{self.name}: \n{data}'
    ...
    def snowiest_location(self) -> dict[date, DailyWeather]:
        ...
        z = 0
        for a in self._histories:
            if self._histories[a].percentage_snowfall() > z:
                z = self._histories[a].percentage_snowfall()
                y = a
        if self._histories[a].percentage_snowfall() == z:
            z = None
            y = None
